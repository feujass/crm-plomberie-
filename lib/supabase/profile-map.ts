...
export function mapSupabaseProfile(row: SupabaseProfileRow | null | undefined): BackendProfile | undefined {
  ...
  return {
    ...
    adresse_ville: (row.adresse_ville as string) ?? undefined,
    adresse_pays: (row.adresse_pays as string) ?? undefined,
    adresse_structure_proposition: (row.adresse_structure_proposition as Record<string, unknown>) ?? null,
    adresse_structure_confirmee_at: (row.adresse_structure_confirmee_at as string) ?? null,
