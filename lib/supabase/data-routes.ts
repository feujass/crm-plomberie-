...
async function replaceDevisLignes(
  ...
  const raw = lignesIn.map((l, i) => {
    ...
    return {
      ...
      tva: Number(row.tva_rate ?? row.tva ?? 10),
      tva_rate: row.tva_rate != null ? Number(row.tva_rate) : row.tva != null ? Number(row.tva) : null,
      ...
      catalogue_ouvrage_id: row.catalogue_ouvrage_id != null ? String(row.catalogue_ouvrage_id) : null,
    };
  });
    ...
    const { error } = await supabase.from("devis_lignes").insert(
      lignes.map((l) => {
        ...
        return {
          ...
          prix_ht: l.prix_ht,
          tva: l.tva,
          tva_rate: (l as { tva_rate?: number | null }).tva_rate ?? l.tva,
          total_ht: l.total_ht,
          ordre: l.ordre,
