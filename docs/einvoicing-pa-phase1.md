...
- **OAuth2 authorization code par artisan** : jetons dans `einvoicing_oauth_tokens`, chiffrés AES-256-GCM. Chaque blob porte un `key_id` (`EINVOICING_TOKEN_ENCRYPTION_KEY_ID`, défaut `v1`). Le callback persiste via `saveConnectionAndTokens`. Après chaque refresh, `withFreshStoredTokens` réécrit le blob sous **verrou** (mutex processus + lease Postgres `refresh_lock_until`) : Super PDP **invalide** l’ancien `refresh_token` (OAuth 2.1 rotatif). Deux refresh concurrents sans lock → `invalid_grant` et déconnexion.
