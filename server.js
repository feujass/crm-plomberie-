...
app.use((req, res, next) => {
  ...
  const m = pathname.match(/^\/(?:api\/)?sign\/([^/]+)\/?$/);
  if (m && (req.method === "GET" || req.method === "POST" || req.method === "HEAD")) {
    req.url = `/api/sign/${m[1]}${search}`;
...
app.get("/api/relances/devis", cronRelancesAuth, (req, res) => {
  void handleRelancesDevis(req, res);
});
app.post("/api/relances/devis", cronRelancesAuth, (req, res) => {
  void handleRelancesDevis(req, res);
});
