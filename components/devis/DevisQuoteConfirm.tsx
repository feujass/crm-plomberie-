...
export function quoteConfirmReady(lines: QuoteConfirmLine[], tva: TvaRateChoice | null): boolean {
  if (tva == null) return false;
  return lines.every((line) => line.designation.trim().length > 0 && parsePrix(line.prix) != null);
}
...
export function DevisQuoteConfirm({
  ...
  function patch(index: number, partial: Partial<QuoteConfirmLine>) {
    ...
    <div className="mt-5 space-y-4 rounded-2xl border border-blue-200 bg-[#EFF6FF] p-4 text-left dark:border-blue-900/50 dark:bg-blue-950/30">
      <div>
        <p className="text-[15px] font-semibold leading-snug text-slate-900 dark:text-slate-100">
          J&apos;ai compris ça, corrige si besoin avant de générer
        </p>
        ...
        {lines.map((line, index) => (
          <div key={`${line.source}-${index}`} className="rounded-xl bg-white p-3 dark:bg-slate-950">
            ...
            <input
              ...
              className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm tabular-nums dark:border-slate-700 dark:bg-slate-900"
            />
          </div>
        ))}
      </div>
      ...
      <Button
        type="button"
        disabled={!quoteConfirmReady(lines, tva) || busy}
        isLoading={busy}
