  ...
  if (isMobile) {
    return (
      <Drawer open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <DrawerContent className="bg-gray-50 p-0 text-gray-900 dark:bg-gray-925 dark:text-gray-50">
          <VisuallyHidden.Root>
  ...
  ({ className, onClick, ...props }, ref) => {
    ...
    return (
      <button
        ...
        type="button"
        className={cx("group inline-flex rounded-md p-1.5 hover:bg-gray-200/50 hover:dark:bg-gray-900", focusRing, className)}
        onClick={(event) => {
          onClick?.(event);
          toggleSidebar();
        }}
...
const SidebarFooter = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => (
  <div ref={ref} data-sidebar="footer" className={cx("flex flex-col gap-2 p-3", className)} {...props} />
));
...
const SidebarHeader = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => (
  <div ref={ref} data-sidebar="header" className={cx("flex flex-col gap-2 p-2", className)} {...props} />
));
...
>(({ children, isActive, icon, notifications, className, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      {notifications ? (
        <span className="inline-flex size-5 items-center justify-center rounded bg-[color:var(--sidebar-accent)] text-sm font-medium text-[color:var(--primary)] sm:text-xs dark:bg-[color:var(--sidebar-accent)] dark:text-[color:var(--primary)]">
          {notifications}
        </span>
      ) : null}
    </a>
...
const SidebarGroup = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => (
  <div ref={ref} data-sidebar="group" className={cx("relative flex w-full min-w-0 flex-col p-3", className)} {...props} />
));
...
const SidebarSubLink = React.forwardRef<
  HTMLAnchorElement,
  React.ComponentProps<"a"> & { children: React.ReactNode; isActive?: boolean }
>(({ isActive, children, className, ...props }, ref) => (
  <a
    ...
    className={cx(
      ...
      "data-[active=true]:rounded data-[active=true]:bg-white data-[active=true]:text-[var(--primary)] data-[active=true]:shadow data-[active=true]:ring-1 data-[active=true]:ring-gray-200 data-[active=true]:dark:bg-gray-900 data-[active=true]:dark:text-[var(--primary)] data-[active=true]:dark:ring-gray-800",
      focusRing,
      className,
    )}
    {...props}
  >
    {isActive ? (
      <div className="absolute left-4 top-1/2 h-5 w-px -translate-y-1/2 bg-[color:var(--primary)]" aria-hidden="true" />
    ) : null}
    {children}
  </a>
));
...
const SidebarMenuSub = React.forwardRef<HTMLUListElement, React.ComponentProps<"ul">>(({ className, ...props }, ref) => (
  <ul ref={ref} data-sidebar="menu-sub" className={cx("relative space-y-1 border-l border-transparent", className)} {...props} />
));
